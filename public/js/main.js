...
editStockButtons.forEach(button => {
  button.addEventListener('click', () => {
    ...
    const shares = Number(button.parentNode.parentNode.parentNode.childNodes[3].childNodes[1].innerText.replaceAll(',', ''));
    const basis = Number(button.parentNode.parentNode.parentNode.childNodes[5].innerText.replaceAll(',', '').slice(1));

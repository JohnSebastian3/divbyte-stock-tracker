  ...
  getDashboard: async (req, res) => {
    try {
      ...
      for (let i = 0; i < stockItems.length; i++) {
        ...
        if (divResponse.data.historical) {
          ...
          if (divResponse.data.historical.length > 1) {
            ...
            firstMonth = Number(
              new Date(currentDivInfo.paymentDate).getMonth()
            );
          ...
          dividendYield = (
            ((dividend * payoutsPerYear) / currentPrice) *
            100
          ).toFixed(2);
          annualDividend += shares * dividend * payoutsPerYear; // quarterly for now, will add algo to change this soon
          if (
            metrics.data[0].dividendPerShareTTM == 0 ||
            metrics.data[0].dividendPerShareTTM === null
          ) {
            dividendYield = "-";
            dividendFrequency = "-";
          }
      ...
      res.render("dashboard.ejs", {
        stocks: stockList,
        totalProfitLoss: formatNumber(totalProfitLoss),
        annualDividend: formatNumber(annualDividend),
